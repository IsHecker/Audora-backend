...
public class ListEpisodesSummaryQueryHandler : IQueryHandler<ListEpisodesSummaryQuery, EpisodesSummaryResponse>
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        var episodes = (await _episodeRepository.GetEpisodesByPodcastIdAsync(request.PodcastId))
            .Paginate(request.Pagination);

        var episodeStatsDict = (await _episodeStatRepository.GetStatsByEpisodeIdsAsync(episodes.Select(ep => ep.Id)))
